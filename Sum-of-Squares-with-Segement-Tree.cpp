//Question: https://www.spoj.com/problems/SEGSQRSS/
...
int a[MAX];
typedef struct node {
	long long sum;//sum of values in that range
	long long sq;//sum of squares in that range
	
	void merge(node l, node r) {//merge lf and rg
		sum = l.sum + r.sum;
		sq = l.sq + r.sq;
	}
}node;
...
int main() {
	...
	for (int l = 1; l <= t; l++) {
		...
		for (int i = 0; i < n; i++)
			scanf("%d", &a[i]);
		N = 2 * pow(2, ceil(log2(n))) - 1;//ceil = ceiling to round up 
		node segment[N];
